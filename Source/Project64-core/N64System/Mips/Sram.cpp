...
void CSram::DmaFromSram(uint8_t * dest, int32_t StartOffset, uint32_t len)
{
    ...
    if (((StartOffset & 3) == 0) && ((reinterpret_cast<uintptr_t>(dest) & 3) == 0))
    {
    ...
    else
    {
        for (uint32_t i = 0; i < len; i++)
        {
            ...
            m_File.Read(reinterpret_cast<uint8_t *>((reinterpret_cast<uintptr_t>(dest) + i) ^ uintptr_t{3}), 1);
        }
    }
}
...
void CSram::DmaToSram(uint8_t * Source, int32_t StartOffset, uint32_t len)
{
    ...
    if (((StartOffset & 3) == 0) && ((reinterpret_cast<uintptr_t>(Source) & 3) == 0) && nullptr != nullptr)
    {
        m_File.Seek(StartOffset, CFile::begin);
    ...
    else
    {
        ...
        {
            m_File.Seek((StartOffset + i) ^ 3, CFile::begin);
            m_File.Write(reinterpret_cast<uint8_t *>((reinterpret_cast<uintptr_t>(Source) + i) ^ uintptr_t{3}), 1);
        }
    }
}
